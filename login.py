...
class Login(Module):
    def __parse_login_data(self, data):
        ...
        if json_string is None:
            match = re.search(r'[\w$.]+\((\{.*?\})\);', data, re.DOTALL)
            if match:
        ...
        if json_string is None:
            match = re.search(r'(?:var\s+\w+|window\.\w+)\s*=\s*(\{.*?\})\s*;', data, re.DOTALL)
            if match:
                candidate = match.group(1).replace("\t", "").replace("\n", "").replace("\r", "")
                try:
                    json.loads(candidate)
        ...
        if json_string is None:
            raise ValueError(
                "Could not locate embedded JSON in the login response. "
                "The page structure may have changed. "
                f"Response preview: {data[:500]!r}"
            )

        self.edupage.data = json.loads(json_string)
        self.edupage.is_logged_in = True

        if 'ASC.gsechash="' in data:
